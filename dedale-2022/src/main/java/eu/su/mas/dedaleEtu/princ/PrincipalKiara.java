 ...
 * <ul>
 * <li> set the ConfigurationFileKiara parameters (and maybe update the files in resources/) {@link ConfigurationFileKiara}</li>
 * <li> create your agents classes and call them in the createAgents method below </li>
 * </ul>
 * @author kg
 *
 */
public class PrincipalKiara {
	...
	public static void main(String[] args){
		...
		if(ConfigurationFileKiara.COMPUTERisMAIN){
			...
			containerList.putAll(createAndConnectContainer(ConfigurationFileKiara.LOCAL_CONTAINER_NAME, ConfigurationFileKiara.PLATFORM_HOSTNAME, ConfigurationFileKiara.PLATFORM_ID, ConfigurationFileKiara.PLATFORM_PORT));
	...
	private static Runtime emptyPlatform(HashMap<String, ContainerController> containerList){
		...
		Profile pMain = new ProfileImpl(ConfigurationFileKiara.PLATFORM_HOSTNAME,ConfigurationFileKiara.PLATFORM_PORT,ConfigurationFileKiara.PLATFORM_ID);
	...
	private static HashMap<String,ContainerController> createContainers(Runtime rt) {
		...
		containerName=ConfigurationFileKiara.LOCAL_CONTAINER_NAME;
		pContainer = new ProfileImpl(ConfigurationFileKiara.PLATFORM_HOSTNAME, ConfigurationFileKiara.PLATFORM_PORT, ConfigurationFileKiara.PLATFORM_ID);
		...
		//create the container0	
		containerName=ConfigurationFileKiara.LOCAL_CONTAINER2_NAME;
		pContainer = new ProfileImpl(ConfigurationFileKiara.PLATFORM_HOSTNAME, ConfigurationFileKiara.PLATFORM_PORT, ConfigurationFileKiara.PLATFORM_ID);
		pContainer.setParameter(Profile.CONTAINER_NAME,containerName);
		System.out.println("Launching container "+pContainer);
		containerRef = rt.createAgentContainer(pContainer); //ContainerController replace AgentContainer in the new versions of Jade.
		...
		containerName=ConfigurationFileKiara.LOCAL_CONTAINER3_NAME;
		pContainer = new ProfileImpl(ConfigurationFileKiara.PLATFORM_HOSTNAME, ConfigurationFileKiara.PLATFORM_PORT, ConfigurationFileKiara.PLATFORM_ID);
		...
		containerName=ConfigurationFileKiara.LOCAL_CONTAINER4_NAME;
		pContainer = new ProfileImpl(ConfigurationFileKiara.PLATFORM_HOSTNAME, ConfigurationFileKiara.PLATFORM_PORT, ConfigurationFileKiara.PLATFORM_ID);
		//pContainer = new ProfileImpl(null, 8888, null);
		pContainer.setParameter(Profile.CONTAINER_NAME,containerName);
		System.out.println("Launching container "+pContainer);
	...
	private static HashMap<String,ContainerController> createAndConnectContainer(String containerName,String host, String platformID, Integer port){
		...
		if (port==null){
			port=ConfigurationFileKiara.PLATFORM_PORT;
		}
	...
	private static List<AgentController> createAgents(HashMap<String, ContainerController> containerList) {
		...
		if (ConfigurationFileKiara.COMPUTERisMAIN){
			 ...
			 */
			c = containerList.get(ConfigurationFileKiara.LOCAL_CONTAINER_NAME);
			Assert.assertNotNull("This container does not exist",c);
			agentName=ConfigurationFileKiara.DEFAULT_GATEKEEPER_NAME;
			try {
				Object[] objtab=new Object[]{ConfigurationFileKiara.ENVIRONMENT_TYPE,ConfigurationFileKiara.GENERATOR_TYPE,ConfigurationFileKiara.INSTANCE_TOPOLOGY,ConfigurationFileKiara.INSTANCE_CONFIGURATION_ELEMENTS,ConfigurationFileKiara.ACTIVE_DIAMOND,ConfigurationFileKiara.ACTIVE_GOLD,ConfigurationFileKiara.ACTIVE_WELL,ConfigurationFileKiara.GENERATOR_PARAMETERS};//used to give informations to the agent
