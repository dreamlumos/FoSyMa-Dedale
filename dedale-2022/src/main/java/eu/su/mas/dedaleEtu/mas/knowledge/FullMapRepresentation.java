...
public class FullMapRepresentation implements Serializable {
	...
	public synchronized boolean addNode(String nodeId, List<Couple<Observation, Integer>> lObservations, long lastVisitTimestamp, long nbVisitTimestamp){
		...
		if (this.g.getNode(nodeId) == null){ // Adding new node
			...
			newNode = true;
			// Keep in mind the following special case: the node the agent starts on will be immediately closed
		...
		if (previousTimestamp == null || lastVisitTimestamp > (long) previousTimestamp) { // I have never visited this node, or if I have, the passed information is more recent
			n.clearAttributes();
...
//		return null;
	...
	/* Only takes into account information that is more recent. */
	...
	public void mergeMap(SerializableSimpleGraph<String, HashMap<String, Object>> sgreceived) {
		...
		for (SerializableNode<String, HashMap<String, Object>> serializableNode0: sgreceived.getAllNodes()){
			
			// Merging node
			mergeNode(serializableNode0);
			String node0Id = serializableNode0.getNodeId();
			
			// Merging edges
			for (String node1Id: sgreceived.getEdges(node0Id)){ // getEdges returns list of nodes that are connected to node0
				addEdge(node0Id, node1Id);
			}
		}
	...
	/**
	 * 
	 * @return true if there exists at least one openNode on the graph 
	 */
